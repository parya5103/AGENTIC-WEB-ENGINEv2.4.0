...
// Prevents re-rendering all complex category UI elements (including expensive object iterations)
// when unrelated states like global telemetry logs update.
    ...
    <div className="p-6 rounded-3xl border border-gray-100 hover:border-blue-200 transition-all group bg-gray-50/30 overflow-hidden relative">
        ...
        <div className="flex items-center justify-between border-t border-gray-100 pt-4 gap-4">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => onRefresh(cat.slug)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all" title="Refresh Content">
              <RefreshCcw className="w-4 h-4" />
            </button>
            <button onClick={() => onDelete(cat.slug)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="Delete Node">
              <Trash2 className="w-4 h-4" />
            </button>
            <a href={cat.url || `/cat/${cat.slug}`} target="_blank" rel="noreferrer" className="px-5 py-2 bg-gray-900 text-white text-[8px] font-black uppercase tracking-[0.2em] rounded-xl hover:bg-blue-600 transition-all flex items-center gap-2 shrink-0">
