...
// ⚡ Bolt Optimization: Wrap CategoryCard in React.memo()
// Prevents O(N) re-renders of the complex target nodes list every time
// new logs stream in from the websocket.
const CategoryCard = React.memo(({ cat, onRefresh, onDelete }: { cat: Category, onRefresh: (slug: string) => void, onDelete: (slug: string) => void }) => {
  const previewImage = cat.posts && Object.values(cat.posts).length > 0 ? (Object.values(cat.posts)[0] as any).imageUrl : undefined;
  return (
    <div className="p-6 rounded-3xl border border-gray-100 hover:border-blue-200 transition-all group bg-gray-50/30 overflow-hidden relative">
      ...
      {previewImage && (
        <div className="absolute top-0 right-0 w-32 h-32 -mr-8 -mt-8 opacity-20 blur-xl group-hover:opacity-40 transition-opacity">
          <img src={previewImage} loading="lazy" className="w-full h-full object-cover rounded-full" alt="preview" />
        </div>
      )}
      ...
      <div className="flex items-center justify-between border-t border-gray-100 pt-4 gap-4">
        ...
        <div className="flex items-center gap-2">
          <button aria-label="Refresh Content" onClick={() => onRefresh(cat.slug)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 focus-visible:ring-2 focus-visible:ring-blue-500 rounded-lg transition-all" title="Refresh Content">
            <RefreshCcw className="w-4 h-4" />
          </button>
          <button aria-label="Delete Node" onClick={() => onDelete(cat.slug)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 focus-visible:ring-2 focus-visible:ring-red-500 rounded-lg transition-all" title="Delete Node">
...
export default function App() {
  ...
  const handleRefresh = React.useCallback(async (slug: string) => {
    if (!user) return;
                      ...
                      <div className="flex-1 overflow-y-auto p-8 pt-4">
                        {categories.length === 0 ? (
                          ...
                          <div className="grid grid-cols-2 gap-6 pb-8">
                              {categories.map((cat) => (
                                <CategoryCard key={cat.slug} cat={cat} onRefresh={handleRefresh} onDelete={handleDelete} />
                              ))}
                          </div>
                        )}
                      </div>
