                      ...
                      <div className="flex-1 overflow-y-auto p-8 pt-4">
                        {categories.length === 0 ? (
                          ...
                          <div className="grid grid-cols-2 gap-6 pb-8">
                              {categories.map((cat) => {
                                // ⚡ Bolt Optimization: Calculate array conversions once per iteration
                                const postsArray = cat.posts ? Object.values(cat.posts) : [];
                                const firstPostImage = postsArray.length > 0 ? (postsArray[0] as any).imageUrl : null;
                                return (
                                <div key={cat.slug} className="p-6 rounded-3xl border border-gray-100 hover:border-blue-200 transition-all group bg-gray-50/30 overflow-hidden relative">
                                    ...
                                    {firstPostImage && (
                                      <div className="absolute top-0 right-0 w-32 h-32 -mr-8 -mt-8 opacity-20 blur-xl group-hover:opacity-40 transition-opacity">
                                        <img src={firstPostImage} className="w-full h-full object-cover rounded-full" />
                                      </div>
                                    )}
                                    ...
                                    <div className="flex items-center justify-between border-t border-gray-100 pt-4 gap-4">
                                      ...
                                      <div className="flex items-center gap-2">
                                        <button onClick={() => handleRefresh(cat.slug)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all" title="Refresh Content">
                                          <RefreshCcw className="w-4 h-4" />
                                        </button>
                                        <button onClick={() => handleDelete(cat.slug)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="Delete Node">
                                          <Trash2 className="w-4 h-4" />
                                        </button>
                                        <a href={cat.url || `/cat/${cat.slug}`} target="_blank" className="px-5 py-2 bg-gray-900 text-white text-[8px] font-black uppercase tracking-[0.2em] rounded-xl hover:bg-blue-600 transition-all flex items-center gap-2 shrink-0">
