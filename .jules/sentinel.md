...
## 2026-05-25 - [MEDIUM] Fix information leakage in Express API endpoints
**Vulnerability:** API endpoints were returning raw error messages directly to the client when an exception occurred (`res.status(500).json({ error: e.message })` or `details: e.message`). Several endpoints also lacked proper try/catch blocks, potentially allowing unhandled promise rejections to crash the server or expose stack traces via default error handlers.
**Learning:** Sending raw error details or internal `.message` properties to the client in HTTP 500 responses creates an information leakage vulnerability. This can inadvertently expose internal architecture, file paths, database schemas, or third-party service details to an attacker, facilitating reconnaissance.
**Prevention:** All Express route handlers must be wrapped in `try/catch` blocks. In the `catch` block, the raw error should be logged internally to the server console (`console.error`), and only a generic HTTP 500 message (e.g., `{"error": "Internal Server Error"}`) should be returned to the client to ensure the application fails securely.
